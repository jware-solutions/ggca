...
type Matrix = Box<dyn Iterator<Item = Vec<f64>>>;
...
fn get_df(path: &str) -> Matrix {
	...
	let parse_record = |record_result: Result<StringRecord, Error>| {
		record_result.unwrap().into_iter().map(|cell| cell.parse::<f64>().unwrap()).collect::<Vec<f64>>()
	};
...
fn main() {
    ...
    let number_of_columns = m1_aux.next().unwrap().len();
