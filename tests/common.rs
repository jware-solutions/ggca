...
pub fn merge_with_adjustment(result: &ResultTupleWithoutAdj, adjustment_result: &Vec<f64>) -> ResultTupleSimple {
	result.iter().zip(adjustment_result.iter()).map(|(elem, adj)| {
		(elem.0.clone(), elem.1.clone(), elem.2, elem.3, *adj)
	}).collect()
} 
